...
export interface CoverflowCarouselProps {
  ...
  /** Fires whenever the centred slide changes, from a drag, arrow key or nav click. */
  onSelectedChange?: (index: number) => void;
  /** Fires when the already-centred card is tapped/clicked (not a drag release). */
...
export function CoverflowCarousel({
  ...
  const onPointerDown = (event: React.PointerEvent<HTMLDivElement>) => {
    ...
    // Read the pressed card's index before capture redirects later pointer
    // (and, in some environments, click) targeting to the frame itself.
    const cardEl = (event.target as HTMLElement).closest<HTMLElement>('[data-card-index]');
  ...
  const endDrag = (event: React.PointerEvent<HTMLDivElement>) => {
    ...
    // A tap, not a drag: settling back onto the same spot would be a no-op,
    // so decide here rather than relying on a separate click — pointer
    // capture on the frame can keep a native click from ever reaching the
    // card underneath.
    if (!drag.moved && drag.cardIndex !== null) {
      if (drag.cardIndex === selected) {
        onActivate?.(drag.cardIndex);
      } else {
        goTo(drag.cardIndex);
      }
      return;
